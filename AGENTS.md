...
- AI provider credentials (`IMAGE_TEXT_API_FORMAT` (currently: `openai`), `IMAGE_TEXT_BASE_URL`, `IMAGE_TEXT_MODEL`, `IMAGE_TEXT_API_KEY`) and the Google Maps key (`GOOGLE_MAPS_API_KEY` / `GOOGLE_MAPS_MAP_ID`) live ONLY on the backend server (`server/server.js`). The frontend exchanges data with the backend through same-origin `/api/*` endpoints (`/api/ai/complete`, `/api/maps/config`, `/api/events`, `/api/mcp`); user data (the event log) is mirrored there under `DATA_DIR`. When the backend is unreachable or unconfigured, the game falls back to `src/ai/mockProvider.js` (and mock maps, IndexedDB-only storage) so it still runs offline / without secrets. Optional `IMAGE_TEXT_SUPPORTS_VISION=1` opts the unit suite into image-input coverage for vision-capable models.
- MCP tool credentials (currently `TAVILY_API_KEY` for `web.search` / `web.fetch`) live only on the backend, next to `IMAGE_TEXT_*`. The tool registry uses the official `@modelcontextprotocol/sdk` (server-side, Node-only); the browser speaks plain JSON-RPC 2.0 over `/api/mcp`. See `docs/mcp.md` for the hybrid SDK adoption pattern.
- All HTTP request/response handling — browser-side and server-side — must log url, action, headers, body, and a curl reconstruction, with auth info (headers, keys in URLs, key fields in JSON bodies) masked.
