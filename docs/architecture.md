...
| Backend AI provider | 5xx mid-session (upstream gateway overload/EOF, server 502 "upstream unreachable") **or a degenerate 200** (`choices:null` / `completion_tokens:0` — the gateway answers HTTP 200 but generates nothing, observed on Qwen3.5 behind newapi-style gateways) | `director.call()` catches the backend throw, logs a masked line with the provider name + `err.status`, and completes the turn with the mock for this call. No retry — retry multiplies pressure on a known-flaky upstream and the mock is deterministic & cheap. 4xx (prompt bug) still throws so the caller sees it. The real-provider contract test in `openaiProvider.test.mjs` (`withUpstreamRetry`) retries the upstream call once when the failure matches a documented upstream-flake shape (degenerate 200 OR 5xx OR transport timeout) — that assertion targets gateway behavior, not our parsing |
