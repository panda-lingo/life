...
| Backend AI provider | 5xx mid-session (upstream gateway overload/EOF, server 502 "upstream unreachable") **or a degenerate 200** (`choices:null` / `completion_tokens:0` — the gateway answers HTTP 200 but generates nothing, observed on Qwen3.5 behind newapi-style gateways); bursts span ~25s so a single retry is insufficient | `director.call()` catches the backend throw, logs a masked line with the provider name + `err.status`, and completes the turn with the mock for this call. No retry — retry multiplies pressure on a known-flaky upstream and the mock is deterministic & cheap. 4xx (prompt bug) still throws so the caller sees it. The real-provider contract test in `openaiProvider.test.mjs` (`withUpstreamRetry`) and the `/api/ai/complete` e2e test in `tests/realai.spec.js` retry up to 3 attempts with 1s/2s backoff when the failure matches a documented upstream-flake shape (degenerate 200 OR 5xx OR transport timeout). After 3 consecutive flake-shaped failures they *skip* rather than fail — those assertions target gateway availability, not our parsing. The sister page-level realai test treats a 5xx from `director.call()` as non-fatal the same way production does. The explore-mode e2e tests (`tests/desktop.spec.js`, `tests/mobile.spec.js`) apply the same tolerance: they probe `/api/healthz` and, when `ai=true`, skip rather than fail if the dialogue input never appears within the 45s budget (the opening-line chain makes two real gateway calls); when `ai=false` (mock) the assertion stays strict |
...
| Google Maps | backend has key, but headless/CI has no WebGL (vector map init fails) | Maps JS SDK falls back to raster tiles and logs a `console.error`; e2e tolerates this expected line |
